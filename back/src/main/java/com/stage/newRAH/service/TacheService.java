...
public class TacheService {
	...
	public ResponseEntity<?> createTache(TacheDTO tacheDTO) {
		Tache nouvelleTache = new Tache();
		 ...
		 TypeTache typeTache = typeTacheRepository.findByLibelle(tacheDTO.getLibelleTypeTache()).get();
		 Projet projet = projetRepository.findByNom(tacheDTO.getNomProjet()).get();
	...
	public ResponseEntity<?> duplicateTache(int idTache, TacheDTO tacheDTO) {
		...
		if (tacheADupliquerOptional.isPresent()) {
			...
			java.util.Date nouvelleDateUtil = calendar.getTime();
	...
	public ResponseEntity<List<Integer>> getListAnneesTaches() {
		...
		listAnneesTaches.add(0); // Pour le cas d'affichage de la durée totale pour un suivi projet 
